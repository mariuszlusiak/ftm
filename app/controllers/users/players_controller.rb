class Users::PlayersController < ApplicationController
  ...
  def index
		page = params[:page] || 1
		@players = @user.players.paginate(:page => page, :per_page => PLAYERS_PER_PAGE)
		...
		respond_to do |format|
			format.html do
				if saved and continue
					redirect_to new_user_player_url(@user)
				elsif saved
      	  redirect_to user_player_url(@user, @player)
      	else
      	  render :action => "new"
      	end
			end
  ...
  def update
    ...
    respond_to do |format|
      if @player.update_attributes(params[:player])
        ...
        format.html { redirect_to user_player_path(@user, @player) }
      else
  ...
  def destroy
    ...
    @player.destroy
    respond_to do |format|
      format.html { redirect_to(user_players_path(@user)) }
			format.js do
    ...
    @player = Player.find(params[:id])
		...
		saved = @player.save
		respond_to do |format|
			format.html { redirect_to user_player_url(@user, @player) }
    end
