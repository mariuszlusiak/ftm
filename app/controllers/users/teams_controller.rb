class Users::TeamsController < ApplicationController
    ...
    respond_to do |format|
			format.html do
				if saved and continue
					redirect_to new_user_team_url(@user)
      	elsif saved
      	  redirect_to user_team_url(@user, @team)
      	else
      	  render :action => "new"
      	end
			end
  ...
  def update
    ...
    respond_to do |format|
      if @team.update_attributes(params[:team])
        ...
        format.html { redirect_to user_team_path(@user, @team) }
      else
  ...
  def destroy
    ...
    @team.destroy
    respond_to do |format|
      format.html { redirect_to(user_teams_path(@user)) }
    end
  end
